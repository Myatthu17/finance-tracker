...
- Auth supports both password and Google login: `POST /api/auth/{register,login}` (bcrypt) and `POST /api/auth/google` (verifies an ID token via `google-auth-library`, matching/creating a user by `google_id` then falling back to `email`). Google-only users have `password = NULL` and `auth_provider = 'google'`; both flows converge on the same `jwt.sign({ userId, email, username })` and `{ token, user }` response shape.
